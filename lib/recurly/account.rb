module Recurly
  class Account < Base
    ...
    def charges(status = :all)
      @charges ||= Charge.list(account_code, status)
    end
    ...
    def credits
      @credits ||= Credit.list(account_code)
    end
    ...
    def transactions(status)
      @transactions ||= {}
      @transactions[status] ||= Transaction.list_for_account(account_code, status)
      @transactions[status]
    end
    ...
    def invoices
      @invoices ||= Invoice.list(account_code)
    end
    ...
    def coupon
      @coupon ||= Coupon.find(account_code)
    end
